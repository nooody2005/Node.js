...
// const locations = JSON.parse(document.getElementById('map').dataset.locations);      // will be moved in index.js 
// console.log(locations);

const displayMap = locations => {
  // all contents from map video will be moved in here
};
...
const updateSettings = async (data, type) => {
  try {
    ...
    if (res.data.status === 'success') {
      (0, _alerts.showAlert)('success', "".concat(type.toUpperCase(), " updated successfully ^_^"));
    }
  } catch (err) {
    (0, _alerts.showAlert)('error', err.response.data.message);
  }
...
var _alerts = require("./alerts");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
const stripe = Stripe('pk_test_51TmC4d6hhWlOMdzdbchsZoCk4jIByNb4NKygXarxXbH0TqJKwHRMym44cgTlFLHMFyi1n08bFtBytqUCxWQ9dfiP00pdfyxhMa'); //we use the public key in front end

const bookTour = async tourId => {
  try {
    // id coming from the tour.pug file
    // 1) get checkout session from API
    const session = await (0, _axios.default)("http://127.0.0.1:8000/api/v1/booking/checkout-session/".concat(tourId));
    console.log(session);
...
if (userPasswordForm) userPasswordForm.addEventListener('submit', async e => {
  ...
  await (0, _updateSettings.updateSettings)({
    passwordCurrent,
    password,
    passwordConfirm // pass data in function 
